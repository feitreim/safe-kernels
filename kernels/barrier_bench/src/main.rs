//! Stage-selection benchmark: the real gemm kernel, `match` over per-stage
//! statics vs stage-indexed arrays (see `src/lib.rs` for the two variants).
//!
//! Both variants run on the same inputs and descriptors. Per-tile math and
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Cross-check first: same inputs + same accumulation order, so the two
    ...
    println!(
        "match over statics    avg={match_ms:.4} ms  {:.1} TFLOP/s",
        tflops(match_ms)
    );
    println!(
        "stage-indexed arrays  avg={arith_ms:.4} ms  {:.1} TFLOP/s",
        tflops(arith_ms)
    );
    println!("slowdown: {:.2}x", match_ms / arith_ms);
    ...
    if mismatches == 0 {
        ...
        println!("✗ outputs differ at {mismatches}/{} u32s", c_arith.len());
        std::process::exit(1);
    }
}
